...
class GroupsData(object):
    ...
    def FromGroupsFile(file, transformed=False):
        ...
        for gid, row in gr_def_df.iterrows():
            ...
            group = Group(gid, row.NAME, row.PROTONS, row.CHARGE,
...
def init_groups_data():
    with resource_stream('component_contribution',
                         '/data/group_definitions.csv') as fp:
        return GroupsData.FromGroupsFile(fp, transformed=False)
