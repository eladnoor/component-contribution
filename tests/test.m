...
%%
fprintf('\n4) Testing getFormulaAndChargeFromInChI() - using ChemAxon script\n')
[formula, nH, charge] = getFormulaAndChargeFromInChI(target_inchis{2});
assert(strcmp(formula, 'H3O4P'));
assert(nH == 3);
assert(charge == 0);

%%
fprintf('\n5) Testing getKeggPkas() - using ChemAxon script\n')
...
%%
fprintf('\n6) Testing getGroupVectorFromInchi() - the group decomposition Python script\n')
